...
class mocksController {
    ...
    static generateData = async (req, res) => {
        ...
        if (isNaN(cantPet) || isNaN(cantUser)) return res.status(400).json({ message: "Validar parametros ingresados" })
        if (!Number.isInteger(cantUser) || !Number.isInteger(cantPet)) return res.status(400).json({ message: "Validar parametros ingresados" })
        if (cantPet < 1 || cantUser < 1) return res.status(400).json({ message: "Ingrese un valor positivo" })
        try {
